/* Ejercicio 7: “La Floreria”
Escribe una función "gestionarFloreria" que realice las siguientes tareas con una lista
predefinida de Flores de flores:
1. Quite los espacios en blanco alrededor de cada flor.
2. Verifique si existe la flor "Margarita" y, si está presente, agregue "Azucena" al final de la
lista.
3. Reemplace todas las flores "Orquídea" por "Clavel".
4. Encuentre el índice de la flor "Girasol" y, si no está presente, agregue "Girasol" al inicio
de la lista.
Devuelva una cadena de flores en orden alfabético separadas por puntos.
 */

let entrada = "Rosa, Tulipán, Orquidea, Lirio";

let gestionarFloreria = (flores) => {
  //Quite los espacios en blanco alrededor de cada flor
  //Transformo a array
  let arrayFlores = flores.split(",");
  let listaFlores = arrayFlores.map((flor) => flor.trim());

  //Verifique si existe la flor "Margarita" y, si está presente, agregue "Azucena" al final de la lista
  if (listaFlores.includes("Margarita")) {
    listaFlores.push("Azucena");
  }

  //Reemplace todas las flores "Orquídea" por "Clavel"
  listaFlores = listaFlores.map((flor) => flor.replace(/Orquidea/g, "Clavel"));

  //Encuentre el índice de la flor "Girasol" y, si no está presente, agregue "Girasol" al inicio de la lista.
  if (listaFlores.indexOf("Girasol" === false)) {
    listaFlores.unshift("Girasol");
  }

  //Devuelva una cadena de flores en orden alfabético separadas por puntos.
  return listaFlores.sort().join(".");
};

console.log(gestionarFloreria(entrada));
