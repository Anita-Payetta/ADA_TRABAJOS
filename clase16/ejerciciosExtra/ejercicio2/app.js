...
let highCost = products.filter(function (product) {
  return product.price >= 50;
});

let highCostName = highCost.map(function (product) {
  return product.name;
});

let totalPrice = highCost.reduce(function (acum, product) {
  return acum + product.price;
}, 0);

console.log(
  `Los productos de costo mayor a $50 son: ${highCostName.join()}. Su precio total es: $${totalPrice}`
);
